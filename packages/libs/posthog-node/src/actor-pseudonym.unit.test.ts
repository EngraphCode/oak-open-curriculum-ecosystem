...
describe('active actor projection', () => {
  ...
  it('never discloses key material through a projection or an error', () => {
    // The key is the secret here — the principal is already pseudonymous input.
    // A distinctive key makes a leak detectable in every encoding a byte array
    // could plausibly be serialised through.
    const secretKey = keyWithByte(0xab);
    const capabilities = unwrapCapabilities(
      createPostHogPseudonymCapabilities(
        validConfig({ activeKeyId: 'leak-probe', keyring: [{ id: 'leak-probe', key: secretKey }] }),
      ),
    );
    const encodings = [
      Buffer.from(secretKey).toString('hex'),
      Buffer.from(secretKey).toString('base64'),
      Buffer.from(secretKey).toString('base64url'),
      Array.from(secretKey).join(','),
    ];

    const serialised = [
      capabilities.active.project('actor'),
      capabilities.deletion.project('actor'),
      capabilities.active.project(''),
      capabilities.deletion.project(''),
    ]
      .map((result) => JSON.stringify(result))
      .join('|');

    for (const encoding of encodings) {
      expect(serialised).not.toContain(encoding);
    }
  });
