...
2. **Degrade honestly.** Honour `NO_COLOR` and dumb terminals: with colour stripped, the output must read identically (rule 1 makes this free). Never encode a value pair (e.g. red-vs-green only) — ANSI palette colours are themed by the _user's terminal_, so contrast is ultimately theirs, not ours.
