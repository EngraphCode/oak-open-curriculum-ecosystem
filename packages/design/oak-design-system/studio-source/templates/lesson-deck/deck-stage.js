  ...
  class DeckStage extends HTMLElement {
    ...
    _neuter(el) {
      ...
      box.style.cssText =
        ...
        (sr ? '' : ';background:rgba(0,0,0,0.06);border:1px dashed rgba(0,0,0,0.15);');
    ...
    _openMenu(i, x, y) {
      ...
      this._menu
        .querySelectorAll(
          '[data-act="skip"], [data-act="up"], [data-act="down"], [data-act="duplicate"], hr',
        )
        .forEach((el) => {
    ...
    _openConfirm(sel) {
      ...
      this._confirm.querySelector('.msg').textContent =
        list.length === 1
    ...
    _moveSlide(i, j) {
      ...
      const cur = this._index;
      const ni =
        cur === i ? j : i < cur && j >= cur ? cur - 1 : i > cur && j <= cur ? cur + 1 : cur;
      const slide = this._slides[i];
