...
- Every colour pairing at least 4.5:1 text / at least 3:1 non-text in **every** theme (light, dark, high-contrast, colour-safe) - validate with `preview/contrast-audit.html`.
- Keep the double focus ring on `:focus-visible` + transparent outline for forced-colors. Never remove or weaken it.
- Hit targets at least 44px (md 48px); `--sm` 36px only for dense desktop UI.
