...
const operationId = 'getAllProgrammesForSubject-getAllProgrammesForSubject' as const;
const name = 'get-subjects-programmes' as const;
const path = '/subjects/{subject}/programmes' as const;
...
if (!primaryResponseDescriptor) {
  throw new TypeError('Missing response descriptor for documented status 200 on getAllProgrammesForSubject-getAllProgrammesForSubject.');
}
 ...
 */
export const getSubjectsProgrammes = {
  invoke: async (client: OakApiPathBasedClient, args: ToolArgs) => {
    const validation = toolZodSchema.safeParse(args);
    if (!validation.success) {
      throw new TypeError(describeToolArgs());
    }
    const endpoint = client["/subjects/{subject}/programmes"];
    const call = endpoint ? endpoint.GET : undefined;
    if (typeof call !== "function") {
      throw new TypeError('Invalid method on endpoint: GET for /subjects/{subject}/programmes');
    }
    ...
    if (!descriptorForStatus) {
      ...
      throw new UndocumentedResponseError(status, 'getAllProgrammesForSubject-getAllProgrammesForSubject', documentedStatuses, responseBody);
    }
