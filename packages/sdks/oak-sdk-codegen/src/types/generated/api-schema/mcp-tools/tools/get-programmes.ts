...
const operationId = 'getAllProgrammesForSubject-getProgramme' as const;
const name = 'get-programmes' as const;
const path = '/programmes/{programme}' as const;
...
if (!primaryResponseDescriptor) {
  throw new TypeError('Missing response descriptor for documented status 200 on getAllProgrammesForSubject-getProgramme.');
}
...
export const getProgrammes = {
  invoke: async (client: OakApiPathBasedClient, args: ToolArgs) => {
    ...
    const endpoint = client["/programmes/{programme}"];
    const call = endpoint ? endpoint.GET : undefined;
    if (typeof call !== "function") {
      throw new TypeError('Invalid method on endpoint: GET for /programmes/{programme}');
    }
    ...
    if (!descriptorForStatus) {
      ...
      throw new UndocumentedResponseError(status, 'getAllProgrammesForSubject-getProgramme', documentedStatuses, responseBody);
    }
