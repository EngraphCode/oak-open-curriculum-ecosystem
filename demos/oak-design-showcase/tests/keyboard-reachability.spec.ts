...
/** Press Tab (or Shift+Tab) up to `presses` times, recording the region
 *  trail. Stops early when focus cycles back to 'outside' after having
 *  entered the page, so trails stay minimal. */
...
async function openForKeyboard(page: Page, path: string): Promise<void> {
