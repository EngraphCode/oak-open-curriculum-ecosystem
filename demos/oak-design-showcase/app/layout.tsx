...
// §4): a plain relative read against the app root — bundler module
// resolution cannot supply a real fs path in a bundled server component, so
// public/oak-theme.js is a tracked copy of the workspace package's file and
// app/oak-assets-parity.unit.test.ts turns any drift into a red test. A raw
// inline <head> script is the only shape that cannot flash: it executes
// during parse, before first paint — next/script beforeInteractive does not
// block hydration and its external fetch can let first paint precede theme
