...
export function PageHeader({
  ...
  return (
    <header className={`border-b-2 border-line ${tint}`}>
      <div className="mx-auto max-w-[1080px] px-6 py-10">
        <p className="mb-2 text-[13px] font-bold uppercase tracking-[0.06em] text-link">
          Curriculum hub
        </p>
        <h1 className="text-[32px] font-bold leading-tight">{title}</h1>
        <p className="mt-2 max-w-[640px] text-[16px] font-light leading-relaxed text-ink-subdued">
          {intro}
        </p>
      </div>
    </header>
  ...
  return (
    <div className="mx-auto max-w-[1080px] px-6 pb-20 pt-8">
      <div className="max-w-[560px] rounded-[10px] border-2 border-l-[6px] border-line bg-accent-subtle-brand px-[18px] py-4">
        ...
        <p className="text-sm font-light leading-[1.55] text-ink-subdued">{children}</p>
      </div>
    </div>
  );
}
