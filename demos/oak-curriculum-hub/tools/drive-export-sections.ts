...
function resolveOutDir(argv: readonly string[]): string {
  const outFlag = argv.indexOf('--out');
  if (outFlag === -1) {
    return path.resolve(TOOLS_DIR, '..', 'demo-evidence', 'export-sections');
  }
...
async function captureSection(page: Page, aside: Locator, target: SectionTarget, outDir: string): Promise<boolean> {
  try {
    ...
    const moduleButton = aside.locator('button[aria-expanded]').filter({ hasText: target.module }).first();
    ...
    await page.locator('#main').screenshot({ path: path.join(outDir, `export-${target.slug}.png`) });
...
async function captureBottomControls(page: Page, outDir: string): Promise<boolean> {
  try {
    ...
    await page.locator('#main').screenshot({ path: path.join(outDir, 'export-bottom-controls.png') });
...
async function main(): Promise<void> {
  ...
  await page.goto(`http://127.0.0.1:${portOf(server)}/Oak%20Course.dc.html`, { waitUntil: 'networkidle' });
