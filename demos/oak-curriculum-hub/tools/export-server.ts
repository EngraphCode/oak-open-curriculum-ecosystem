...
function handleStaticRequest(
  ...
  // One stat with throwIfNoEntry:false, not existsSync-then-statSync: the
  // export tree is regenerated by the capture tooling, so a file can
  // vanish between two probes — and every throw in this listener kills
  // the run raw, outside any runTool boundary. isFile() (not
  // !isDirectory()) also refuses FIFOs, whose open would block a libuv
  // thread indefinitely.
  const stat =
    resolved === undefined ? undefined : fs.statSync(resolved, { throwIfNoEntry: false });
  if (resolved === undefined || stat === undefined || !stat.isFile()) {
    res.writeHead(404);
    res.end();
    return;
  }
  ...
  // listener. The headers are already written, so end the response — the
  // client sees a truncated body and the capture self-checks catch it.
  stream.on('error', () => {
    res.end();
  });
  stream.pipe(res);
