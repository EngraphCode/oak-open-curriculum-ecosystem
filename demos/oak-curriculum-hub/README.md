...
| `tools/`                                  | Evidence + verification tooling (live-demo capture, canonical-target rendering, the export/live section arms, the 320px reflow gate, the token-fidelity audit, and the fidelity review). Strict TypeScript run via the `tool:*` package scripts, e.g. `pnpm --filter @oaknational/oak-curriculum-hub tool:fidelity`.                                                                                                                          |
