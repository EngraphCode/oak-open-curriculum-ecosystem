...
| `tools/`                                  | App-local evidence + verification tooling (live-demo capture, canonical-target rendering, the export/live section arms, the 320px reflow gate, the token-fidelity audit, and the fidelity-review CLI). The shared core — diff engine, dev-server lifecycle, path guards, report renderer, register schema — comes from [`@oaknational/fidelity-review`](../../packages/libs/fidelity-review/README.md). Strict TypeScript run via the `tool:*` package scripts, e.g. `pnpm --filter @oaknational/oak-curriculum-hub tool:fidelity`. |
