...
export function displayArea(area: string): string {
  // String surgery, not regex: the qualifier is a literal suffix in the
  // corpus, and the old `\s*` pattern carried super-linear backtracking risk.
  const suffix = '(CfUs)';
  return area.endsWith(suffix) ? area.slice(0, -suffix.length).trimEnd() : area;
}
