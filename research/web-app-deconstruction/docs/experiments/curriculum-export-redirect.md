...
The checked-in Jest characterization fixture:
...
Its exact body is
[`missing-mv-refresh.test.ts.template`](../../packages/research-evidence/fixtures/curriculum-export/missing-mv-refresh.test.ts.template).
The two import placeholders are replaced with the selected OWA checkout's paths
when the harness stages the test in the operating system's temporary directory.
From the deconstruction repository, run:

```sh
pnpm --filter @oaknational/research-evidence curriculum-redirect
```

The command uses OWA's installed Jest and configuration, records the OWA
revision and worktree status, normalizes machine paths in its JSON output, and
restores or removes the `.env.local` file created while that configuration
loads. `--owa` selects another checkout and `--output` retains raw JSON; see the
harness [README](../../packages/research-evidence/README.md).
...
The checked-in harness reproduced both assertions again at the pinned revision.
