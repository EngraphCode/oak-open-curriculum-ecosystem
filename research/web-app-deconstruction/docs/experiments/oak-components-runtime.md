...
measurement code are now checked in under
[`packages/research-evidence`](../../packages/research-evidence/README.md); only
their build output is temporary.
...
From the deconstruction repository, with the pinned OWA and Components
...
The first command reproduces artifact compression, runtime-name counts, isolated
Rollup/Terser results and Node compatibility probes. The second builds the five
checked-in Next fixtures. Both emit normalized JSON and record source revisions,
package/tool versions and initial source-worktree status. They accept `--owa`,
`--components`, `--scenarios` and `--output` as documented in the harness
[README](../../packages/research-evidence/README.md).
...
and 251 kB first load, while two runs of the checked-in fixture reported the same
