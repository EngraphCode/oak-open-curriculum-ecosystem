...
It reads the two source repositories without changing them and emits JSON with
their revisions, package versions and initial worktree status. The generated
export list is raw evidence rather than a reviewed conclusion and is not
committed; see the harness
[README](../../packages/research-evidence/README.md) for output handling and
path overrides.
