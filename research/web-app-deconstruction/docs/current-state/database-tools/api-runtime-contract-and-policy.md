...
**Observed experiment:** The committed local
[`openapi-oce-contract-comparison`](../../../packages/research-evidence/scripts/openapi-oce-contract-comparison.ts)
fixes the child environment, verifies that oak-openapi's installed pnpm lock
snapshot matches its committed lockfile, blocks common Node network entry points,
generates the pinned provider document in memory, filters it through the served
Swagger route, reads OCE's cached input from its pinned Git object, and compares
canonical projections. Both documents contain 32 paths, 32 GET operations and 32
component schemas. Their structural projections have the same SHA-256 digest.
