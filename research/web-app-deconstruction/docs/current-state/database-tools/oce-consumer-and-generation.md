...
`downloadsAvailable` description. The committed
[`openapi-oce-contract-comparison`](../../../packages/research-evidence/scripts/openapi-oce-contract-comparison.ts)
reproduces the import, sanitisation, semantic/structural projections, SHA-256
digests and exact difference paths while recording the Node/lockfile boundary.
...
**Inference:** OCE is structurally aligned with the pinned provider now, but its
...
count and exact paths are retained by the revision-exact
