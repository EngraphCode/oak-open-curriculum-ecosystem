# Eve + MCP agentic chat experience — one peer demo in the Oak Innovation Kit portfolio
...
The stronger purpose of this demo is to contribute one bounded piece of evidence to the portfolio-level question of whether Oak has become a **value-preserving, recomposable public capability system**:
...
This report keeps four related objects distinct:

1. **The specimen — the agentic chat experience:** a durable, human-governed curriculum inquiry centred on an evolving artefact rather than only a chat transcript.
2. **The shared substrate touched by this demo — the current Innovation Kit hypothesis:** the specific OCE capabilities, Oak Design System primitives, agent infrastructure and delivery practices the chat reuses or exposes.
3. **The peer demos — repeated consumers and probes:** implemented demos test distinct regions of possibility and challenge candidate commonality.
4. **The portfolio and evidence ledger — the comparative programme:** it records vertical, horizontal and longitudinal evidence without treating planned work as proof.

The strategic hypothesis across those objects is public capability that travels: Oak knowledge remains recognisable and inspectable inside AI environments Oak does not control; challenges route to the legitimate source authority; tested provider seams remain substitutable; and teacher decision authority is encoded explicitly. Evidence that professionals understand and exercise that authority requires a professional probe.
...
| Public value | Why does this matter in an AI-mediated world? | Public knowledge should remain active, attributable, contestable and reparable, not merely published or ingested. | Treat the demo as a public-capability prototype, not merely generic AI spectacle or product-value evidence. |
...
The purposes form a causal ladder. Higher levels depend on lower ones but are not proven by them.
This is a theory-of-change ladder, not a maturity or delivery sequence for the peer demos.

1. **Technical reuse:** existing SDK, search, graph, MCP and UI capabilities work outside their original surfaces.
2. **Value-preserving composition:** those capabilities can be recombined without duplicating domain logic or losing provenance and policy, while accessibility requirements and tested primitives carry forward and conformance is reverified.
3. **Agentic product material:** Eve, MCP Apps and WebMCP offer candidate mechanisms for new interaction forms—durable inquiry, live workspace action and portable rich UI—not merely another chat implementation.
4. **Possibility revelation:** demonstrations make invisible relationships and dormant capabilities perceptible, memorable and emotionally salient; they expand what audiences believe can be built or understood.
5. **Responsible imagination at lower cost:** product, design, curriculum and engineering teams can instantiate more divergent possibilities before committing heavily.
6. **Faster contact with reality:** credible concepts become boundary objects through which teachers and stakeholders can expose assumptions, disagreements and new ideas.
7. **Better institutional learning:** demonstrations change understanding or decisions; validated general improvements return to shared capabilities; failed ideas and their evidence are retained.
8. **Professional control and possible capability development:** the selected professional can understand, interrogate, adapt and decide rather than merely receive an answer; durable capability development requires later transfer or delayed evidence.
9. **Product boundary change:** Oak can participate as a capability inside many workflows and hosts instead of requiring every user to visit an Oak-owned destination.
10. **Institutional and ecosystem option value:** Oak and independent builders can change models, hosts and experience shapes without reconstructing curriculum meaning.
11. **Public epistemic infrastructure:** publicly funded knowledge remains identifiable, actionable and challengeable, so public institutions, professionals and builders can exercise agency in AI-mediated work.
12. **Mission contribution:** if adoption and beneficiary bridges hold, this may support better teaching and pupil outcomes and help narrow disadvantage.

The deepest concise formulation is:

> We are testing whether trusted public knowledge can remain recognisable, contestable, adaptable and human-governed when it becomes active inside agentic systems we do not control, with correction routed to the legitimate source authority.
...
## 3. Three distinct durable layers: Innovation Kit, demo portfolio and evidence ledger

The repository already contains this direction. OCE’s strategy names the **Oak Innovation Kit** as the tools and knowledge needed to create production-standard Oak products at pace. Historical Curriculum Hub plans provide evidence for the reuse discipline; this report does not assume that Curriculum Hub and the user-defined Curriculum Studio are the same demo. Those plans separate three co-equal programme workstreams, not the repository’s now-four value streams:
...
One specimen is not enough to prove portability or commonality, but consumer count is not a prerequisite for deliberate Innovation Kit ownership. The current [research charter](../../../research/web-app-deconstruction/docs/research-charter.md) makes the stronger distinction: capability belongs in the framework when meaning, semantic authority, invariants, isolation, lifecycle or assurance justify that boundary; unlike implemented consumers then test the reach of the portability and commonality claims. The next UI should cost a fraction of the first without making a second consumer the source of architectural truth.
...
Terminology matters: the **Oak Design System** is shared design substrate; the **design-system showcase** is the peer demo that tests its breadth and presents it compellingly; the **Oak Innovation Kit** is the broader capability, design, delivery and experimentation substrate.

### Showing what is possible is a first-class outcome

Some demonstrations should intentionally optimise for revelation, inspiration and impact. Data structures, APIs and tool definitions are difficult to understand in the abstract; an excellent interactive experience can let people *feel* the shape and reach of the capability. Being impressive can earn the attention needed for deeper understanding, generate new questions, attract collaborators and raise the ambition of subsequent work.
...
Keep three analytical levels separate:

1. **Innovation Kit:** shared curriculum capabilities, contracts, design, delivery and experimentation substrate.
2. **Demo portfolio:** independent instruments that stress different regions of that substrate and possibility space.
3. **Evidence ledger:** what each demo actually demonstrates, contradicts or leaves unresolved.
...
Embedded reuse must not be double-counted. If Curriculum Studio and chat both use semantic search, that is evidence of reuse and ecological applicability; it is not two independent validations of search relevance. Planned demos contribute intent, not demonstrated evidence. The portfolio register should therefore use explicit states such as `identified`, `exploratory`, `implemented`, `deployed` and `user-evaluated`.
...
- **Retention:** general improvements return to SDKs, tool contracts, design primitives, adapters and delivery practice; product-specific code remains local.
...
**Acceptance:** every in-scope claim has a collection method, threshold and losing condition; the portfolio register gives the demo an explicit status and does not treat planned sibling demos as completed evidence.
...
Update the portfolio evidence ledger with:

- the chat demo’s actual status and evidence class;
...
- Kit placement decided from meaning, semantic authority, invariants, isolation, lifecycle and assurance rather than consumer count;
...
- active effort split into shared substrate, chat-specific experience, assurance, promotion and maintenance.
...
### Stage 6 — portfolio-level independent-consumer proofs
...
- this demo’s contribution to portfolio-level Innovation Kit evidence; and
...
| The chat demo reveals possibility | Intended audiences accurately understand newly visible capabilities, remember the real mechanism and generate materially new questions, ideas or intended uses | They remember only “AI chat” or visual polish, misunderstand what the system can do, or the impressive behaviour is staged rather than real |
...
| The chat demo produces learning | Understanding expands, new ideas emerge, its named uncertainties resolve, its claims are dispositioned, negative results are retained and candidate shared improvements are recorded | It produces neither the intended understanding nor a technical disposition, hides friction, or leaves only polished code |
...
The portfolio-level Innovation Kit claim is judged later from vertical, horizontal and longitudinal evidence across the real demos. It is neither proved nor falsified by this chat demo alone.
