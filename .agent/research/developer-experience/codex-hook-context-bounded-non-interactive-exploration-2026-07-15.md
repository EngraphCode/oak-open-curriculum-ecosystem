...
disabled. The associated evaluation plan
(`codex-hook-semantic-classifier-evaluation.plan.md`, preserved on PR #705's branch at
freeze; the old `.agent/plans/agent-tooling/` layout it targeted was refounded) was
authoritative for its own qualification thresholds, activation, acceptance, and non-goals;
its still-live intent is carried by this report's closing status above. **Lane disposition
(dated addendum, 2026-08-02): the experiment lane is RETIRED at owner ruling.** This report
and the labelled corpora beside it
(`codex-hook-review-corpora/` — calibration + three held-out difficulty tiers) are the
durable extraction; any future fast semantic pre-tool-review attempt starts from these, on a
fresh branch, honouring the pinned-version re-derivation discipline. Full estate: PR #705
(closed; commits reachable via the PR ref).
