# Governance Planes and Supervision

Use this lane for source material about repo-native governance planes,
trust boundaries, runtime supervision, contribution contracts, and adjacent
institutional control surfaces around agent execution.

## Best Starting Points

- [repos-as-governance-planes.md](./repos-as-governance-planes.md) —
  landscape snapshot of 15 governance-plane projects.
- [governance-plane-direction-of-travel.md](./governance-plane-direction-of-travel.md) —
  trajectory companion to the snapshot: roadmaps, RFCs, design pivots
  for the same 15 projects, with five cross-project trajectory
  patterns and per-project repo-local implications.
- [governance-concepts-and-mechanism-gap-baseline.md](../../../analysis/governance-concepts-and-mechanism-gap-baseline.md)
- [practice-aligned-direction-and-gap-baseline.md](../../../analysis/practice-aligned-direction-and-gap-baseline.md) —
  direction-of-travel comparison matrix (companion to the
  governance-concepts baseline above).
- [governance-concepts-and-integration-report.md](../../../reports/agentic-engineering/deep-dive-syntheses/governance-concepts-and-integration-report.md)
- [governance-concepts-and-agentic-mechanism-integration.plan.md](../../../plans/agentic-engineering-enhancements/current/governance-concepts-and-agentic-mechanism-integration.plan.md)

## Feeds

- [governance planes deep dive](governance-planes-trust-boundaries-and-runtime-supervision.md)
- [hub README](../README.md)
- [formal synthesis lane](../../../reports/agentic-engineering/deep-dive-syntheses/README.md)
