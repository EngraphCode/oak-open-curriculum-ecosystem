...
7. **Cost/abuse controls** — reuse the MCP app's `express-rate-limit` / rate-limiter factory; set Gateway budget alerts.
...
- The Gateway model slug is dot-separated (`anthropic/claude-sonnet-5`); the hyphenated `claude-sonnet-4-5` is both legacy and the wrong slug format.
