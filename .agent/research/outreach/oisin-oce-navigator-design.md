...
## TL;DR
- Host on Vercel as a **Next.js App Router** app using the official `@vercel/slack-bolt` adapter, which uses Fluid compute's `waitUntil` to acknowledge Slack inside its 3-second window while the model call continues in the background. Next.js is settled because the adapter is Web-Request-native (`export const POST = createHandler(app, receiver)`) — Express is not one of its targets. Reuse only the genuine shared `@oaknational/*` packages (result/env/logger/sentry-node/type-helpers/build-metadata); rate limiting is rebuilt as a durable-KV limiter (not `express-rate-limit`), Clerk is Ask-Oak-only, and the Express router does not transfer — none of those three is an Oisín reuse. Socket Mode is not usable on Vercel (no long-lived process); it stays a local-dev convenience only.
...
**Workspaces depended on.** Existing (all already consumed by the MCP app, so a proven set): `@oaknational/result`, `env`, `env-resolution`, `logger`, `sentry-node`, `type-helpers`, `build-metadata` (this is the genuine-reuse set; `@oaknational/observability` is *not* a straight reuse — the Next.js Sentry wiring is decided in WS8); the Clerk stack (`@clerk/backend`, `@clerk/mcp-tools`) for Ask Oak; optionally `@oaknational/curriculum-sdk` if Ask Oak calls some endpoints directly rather than via MCP. New: `packages/libs/slack-assistant` (the framework); and, only at a second consumer, a thin `ai-gateway` wrapper lifted out of it (per `consolidate-at-second-consumer` — don't extract pre-emptively).

**The three seams.**
- *Reusable everywhere (org-agnostic, publishable):* Bolt + `@vercel/slack-bolt` ack/`waitUntil` wiring; the invocation mechanics (mention, slash routing); the AI-SDK bounded tool loop, model-slug handling, streaming; MCP-client attachment given a URL + auth + toolset filter; **the PII egress boundary**; mrkdwn rendering, the LLM-content disclaimer; signature verification, rate limiting, the Sentry redaction barrier. (A feedback affordance is **deferred** from v1 — see the plan's Non-Goals — so it is not part of the v1 framework surface.)
...
Block Kit (suggested prompts, mrkdwn) is not React, so v1 is React/Next-free and not blocked by the missing resources.
...
4. **No content persistence** — nothing logged/persisted outside Slack (this holds independently of ZDR); ZDR on as a beneficial model-provider-retention control. Secrets (Slack tokens, Gateway key, the GitHub PAT with private `oak-skills` read, Ask Oak's refresh token) encrypted, minimally scoped, rotated.
...
6. The handler posts the answer as Slack `mrkdwn`, appends an LLM-content disclaimer, and sets the thread title. (A feedback affordance is deferred from v1 — see the plan's Non-Goals.)

**Hosting & runtime (Vercel).** A **Next.js App Router** app on Vercel (settled). The Slack request URL is an App Router route handler wired to Bolt through `@vercel/slack-bolt` (`export const POST = createHandler(app, receiver)`), reusing only the genuine shared `@oaknational/*` packages (not `express-rate-limit`, not the Express router, and not Clerk — Clerk is Ask-Oak-only); the route sets `maxDuration` high enough to cover model **plus live GitHub tool** latency after the fast ack, and Fluid compute keeps the function running for `waitUntil` (default-on for new projects). Socket Mode cannot run on Vercel; keep it only for a local dev process if wanted.
...
## Recommendations
1. **Host both apps as Next.js App Router apps on Vercel with `@vercel/slack-bolt`** (settled — the adapter is Web-Request-native), reusing only the genuine shared `@oaknational/*` packages (rate limiting rebuilt on a durable KV, not `express-rate-limit`; Clerk is Ask-Oak-only; the Express router does not transfer). Set `maxDuration` to cover model plus live tool latency; HTTP request URL, no Socket Mode.
...
4. **Enforce the pragmatic PII boundary in the framework** (§Security): strip identity, scrub PII on prompt and tool args, no content in logs/Sentry/KV, egress allowlist, ZDR on (beneficial — the invariant does not depend on it).
