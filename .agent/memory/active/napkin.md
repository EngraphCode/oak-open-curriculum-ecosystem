...
## 2026-09-02 08:56Z (date -u read; Pegasus mends Aphelion, 4cbd16) — cache-vs-live spec drift census, first-hand, for the disable-dead-tools lane

- Cache 0.7.0 (34 paths) vs live 0.11.0 (32 paths, unchanged since the 2026-08-21
  measurement). After $ref resolution + prose stripping: ZERO response-schema
  differences on the 32 shared paths (independently re-derived; matches the plan's
  2026-08-19 claim). Real deltas: (1) all 32 operationIds renamed (dash dropped:
  `getKeyStages-getKeyStages` → `getKeyStagesGetKeyStages`) — tool names derive from
  path+method so no tool renames; no non-test product code pins the old ids (21 hits,
  all generator test fixtures); (2) `/key-stages/{ks}/subject/{subject}/assets` and
  `.../units` gain `limit`+`offset` → `isPaginatedQueryMetadata` will mark two more
  tools paginated at regen (composes with the in-flight Link-header pagination lane);
  (3) `/lessons/{lesson}/assets/{type}` gains a documented 302 for video — SKIPPED
  path, and the asset proxy already fetches with `redirect: 'follow'`, so doc catching
  up with runtime; (4) 184 prose deltas across 24 operations: 122 example changes —
  74 examples REMOVED upstream (mostly response-property examples on lesson/unit
  summary), 17 added, 31 changed (ks1→ks3, science→english swaps) — plus 54 parameter
  descriptions, 5 response-property descriptions. Generated tool files DO carry spec
  examples into input schemas, so the resync degrades some agent-facing examples;
  the path-keyed `param-description-overrides.ts` layer + its removal-condition test
  is the estate's re-adjudication mechanism. Tool census implication for Aakesh's
  "other changes to reflect": nothing structural rides with the changelog removal;
  the prose class is the refresh's real review load next week.
- Hook catch worth keeping: Write of a scratchpad script carrying an absolute
  home-rooted path is blocked by the machine-local-path fingerprint even OUTSIDE the
  repo; argv-passed paths are the portable shape (second instance; first in the
  2026-08-09 napkin entry).

## 2026-09-02 (Pegasus mends Aphelion, 4cbd16) — layered cache-vs-live spec audit: the byte-identical claim was response-schema-scoped; parameters DID drift

- Ran the owner-requested (via Aakesh's Slack ask) full-context drift audit ahead
  of the disable-dead-tools lane. The repo's own `ci-schema-drift-check` reports
  version drift only (0.7.0-3b4b01e6 → 0.11.0, advisory). A layered diff
  (paths / operations / parameters / ref-resolved response schemas / prose)
  found what the 2026-08-21 "byte-identical on all 32 shared paths"
  verification could not see — that claim was scoped to 200-RESPONSE SCHEMAS
  after prose-stripping, and it still holds; but: (1) upstream ADDED
  `limit`+`offset` params to `/key-stages/{ks}/subject/{s}/units` and
  `.../assets` (two more endpoints paginated — intersects the in-flight
  Link-header pagination lane); (2) 184 prose leaves changed across 24 shared
  operations (mostly regenerated examples, science→english corpus churn; no
  semantic contract change found on sampling); (3) one structural response
  diff on the non-tool SKIPPED path `/lessons/{lesson}/assets/{type}` (302 now
  documented). Path set: exactly 2 removed (`/changelog`, `/changelog/latest`),
  0 added — the broken-tool count is exactly 2, triple-confirmed (38/40 sweep
  pass 2026-09-01; fresh 404s + 200 control 2026-09-02; path diff).
- Class lesson: a drift verification's scope clause ("response schemas,
  prose-stripped") bounds what its "in-sync" verdict covers — re-widen the
  instrument before reusing the verdict for a NEW question (here: "what else
  must be reflected in the tools?"). Differ script argv-portable at
  scratchpad spec-diff.py; findings route to the disable-lane PR + MCP-630/653
  at owner approval.

## 2026-09-02 ~10:5xZ (Pegasus mends Aphelion, 4cbd16) — disable-dead-changelog-tools lane: commit 62fc40ffa landed; four instrument lessons

- Lane executed end-to-end in worktree fix/mcp-653-disable-dead-changelog-tools
  (owner word via Slack/Aakesh + Luke): DEFERRED_PATHS entries for the dead
  /changelog pair (self-retiring — applyDeferredPaths throws on absent paths,
  so the MCP-630 refresh forces their deletion), regen (generator DELETES
  orphaned tool files — cleaner than feared), forced true-ups (served-surface
  rows, PUBLIC_TOOLS, five test surfaces retargeted to get-rate-limit, UAT
  probe swap, tool table 40→38), and the FULL content-audit re-attestation:
  8 rows retired via post-baseline lineage, A002 addition anchor updated,
  C677/C678 generated-item anchor overrides, 8 delta-review ledger entries
  (hashes computed via the repo's own semanticSourceSha256 — never typed from
  memory), artefacts refreshed via --refresh-anchors. Live proof: local server
  lists 38 tools, changelog absent, 38/38 green with valid args.
- HOOK ENV LESSON (bit twice): git hook children inherit the CALLING
  process env — a commit run without NODE_OPTIONS/TURBO_CONCURRENCY let the
  hook's eslint OOM (exit 134, app lint) and turbo saturate the 4-core box
  (load 34), flaking two canonical-origin integration tests that pass under
  caps. Cure: export TURBO_CONCURRENCY=4 NODE_OPTIONS=--max-old-space-size=4096
  on the `git commit`/`git push` command line on this machine, and budget
  >10min (Bash tool cap killed one attempt at 600s mid-hook; background the
  commit instead).
- TURBO-CACHE LESSON: direct `pnpm exec vitest run` proves suites fast but
  does NOT warm the turbo cache the hook reads — the hook re-ran ~6k tests
  cold. When a hook chain is imminent, run gates THROUGH turbo tasks.
- MAX-LINES AT THE LEDGER: audit delta-review ledgers sit near the 250-line
  cap; my entries breached two (252/276). Cure per the directory's own
  convention: new family file current-source-delta-reviews-sdk-generated-registry.ts;
  app.ts left at exactly 250 — AT CAPACITY, the next toucher must split it
  (flagged for MCP-630's cleanup changeset).
- MID-RUN READS (self-caught, again): `git diff` during a running build read a
  half-cleaned tree (132 files "changed", 58k deletions — phantom); and two
  `| tail` pipes ate a red exit + truncated a background log to 8 lines.
  The one-law's standing cures applied on the retry each time.
- Held for owner word: push + draft PR + MCP-653/630 ticket updates. Worktree
  kept; lane claim still open; commit-window claim closed at 10:57Z.

## 2026-08-28 — EEF reactivation lane (Dynamo guards Fumes / Sequoia tracks Phloem, PR 919) [reinstated 2026-09-02, see below]

- Whole-tree hook on the 8GB/4-core WSL host: unbounded turbo OOM-kills eslint
  and timeout-fails unrelated jsdom suites (oak-curriculum-hub, clerk-public-
  surface) — false test failures. Cure that worked twice: TURBO_CONCURRENCY=2
  NODE_OPTIONS=--max-old-space-size=4096 on the commit invocation. Candidate
  for start-right §7 host-health guidance.
- gh multi-account: active account silently switched to a non-Oak login
  (luke-openresource) between sessions; push 403s name the wrong-account —
  check `gh auth status` before diagnosing permissions. `gh auth switch
  --user luke-arnold-oak` cures.
- Restarted session landed cwd in the primary checkout and `git push` there
  pushed the checkout's own branch (docs lane) — harmless non-fast-forward,
  but the worktree-residency rule exists for exactly this; re-enter via
  EnterWorktree, not cd.
- served-surface flip checklist (beyond the two rows): content-audit anchor +
  delta hash re-pin, regenerated truth set, served-tool-table regen, UAT guide
  counts in THREE places (§0 table, §8, Appendix A), and the ADR-205 public
  allowlist for any newly-live static resource (Copilot caught the allowlist;
  UAT §0/§A counts survived my own sweep).

## 2026-08-28 (Myrtle mends Xylem, 98e4e9, MCP-638 plugin-repoint lane) — sweep blind spot + fresh-worktree signatures [reinstated 2026-09-02, see below]

- ENDPOINT SWEEPS NEED A BARE-HOST PASS: two independent instruments (my
  all-filetype grep and an opus reviewer's sweep) both keyed on the
  `host/path` literal (`www.thenational.academy/mcp`) and both missed
  `docs/middleware-chain.md:168-175`, which names the host WITHOUT the
  path while still calling it the canonical deployment. Copilot's PR
  round caught it (#940, round 1). Cure habit: any URL/endpoint sweep
  runs twice — once with the path, once bare-host — and instrument
  DIVERSITY (a reviewer with different patterns) is what caught what
  same-keyed instruments structurally could not.
- FRESH-WORKTREE ENV GAP HAS A MISLEADING SIGNATURE: a missing
  `.env.local` in a worktree presents at pre-push as Playwright
  "Timed out waiting 60000ms from config.webServer" — no env error
  anywhere in the failure tail. Read as server-never-booted, check env
  files FIRST (start-right §8's carry step covers it; the timeout shape
  is the new part worth knowing).
- 8GB-HOST GATE CALIBRATION, worked: full-parallel sweep produced one
  real OOM (mcp app eslint, exit 134 — cure `NODE_OPTIONS
  =--max-old-space-size=4096`) and three phantom test reds that each
  passed solo (load starvation); `TURBO_CONCURRENCY=4` + the heap flag
  made the identical sweep green end to end. Solo re-run is the cheap
  phantom-vs-real discriminator before diagnosing any parallel-sweep red.

## 2026-09-01/02 (Foehn rides Sundog, e33d95) — payload audit → pagination-echo lane; scratchpad-loss lesson

- Ran the full served tool surface (40 tools, 68 calls) against a local
  `dev:observe:noauth` boot to measure payload sizes first-hand; findings in
  the session report and PR #949 (pagination echo: Link-header signal into
  paginated tool results, MCP-487). Instrument notes: `pnpm dev` FORCES auth
  on; `dev:observe:noauth` is the only no-auth boot. The upstream API 403s
  python-urllib's default User-Agent; any UA string cures it.
- Own-mistake log (verification-read variant of the one-law): after a silently
  failed `cd && sed` chain, the verification `grep -n ELASTICSEARCH .env.local`
  printed the ES API key verbatim into the session transcript — every other
  read used a redaction filter except the one that proved the cleanup. Cure,
  standing: verification reads of secret-bearing files go through the
  redaction filter too. ES key rotation surfaced to the owner.
- Own-mistake log (important-state-not-in-temp-files, demonstrated): banked
  the docs-branch napkin's uncommitted entries to the session scratchpad
  before `git restore`; the harness rotated the session overnight and the
  scratchpad was CLEANED — the banks died in /tmp exactly as the rule
  predicts. Recovered verbatim from the conversation record (the entries had
  been read/authored in-context) and reinstated above/here. Cure, standing: a
  bank whose debt outlives the current sitting goes to a durable home (the
  napkin itself, or the per-project memory dir) at banking time, never to
  scratchpad.
- These two 2026-08-28 sections above are reinstated copies: they were
  uncommitted working-tree dirt on docs/windows-via-wsl, banked and restored
  away on 2026-09-01 to unblock the pagination lane's branch switch, then
  lost with the scratchpad. Their content is byte-faithful to the
  conversation record's read of the original working tree.

## 2026-09-01/02 (Foehn rides Sundog, e33d95, pagination-echo lane) — MCP tool payload measurement + echo landing notes

- Ran the full served tool surface (40 tools, 68 calls) against a local
  `dev:observe:noauth` boot to measure payload sizes first-hand; findings in the
  session report and PR #949 (headline: unfiltered browse-curriculum ~68k tokens,
  unfiltered get-sequences-assets ~477k tokens, REST-passthrough pagination
  signals dropped at the MCP layer while descriptions pointed agents at the
  upstream Link header). The pagination echo landed as acdd8d434 under MCP-487.
- Instrument notes: `pnpm dev` FORCES auth on (http-dev-contract.ts sets
  DANGEROUSLY_DISABLE_AUTH=false for dev mode) — `dev:observe:noauth` is the
  only no-auth boot. The upstream API 403s python-urllib's default User-Agent.
- Own-mistake log (one-law family, verification-read variant): a verification
  `grep -n ELASTICSEARCH .env.local` printed the ES API key verbatim into the
  session transcript after a silently failed `cd && sed` chain. Cure, standing:
  verification reads of secret-bearing files go through the redaction filter
  too. ES key rotation surfaced to the owner.
- Scratchpad-bank loss, first-hand (important-state-not-in-temp-files): the
  napkin banks lived in the session scratchpad; an overnight session rotation
  cleaned it. Content survived only because every line was verbatim in the
  conversation. Cure, standing: banks of repo-surface content go to a durable
  home (the repo or ~/.claude project memory), never only the scratchpad.

## 2026-09-02 11:2xZ (Mistral turns Halo, 0390d6) — full-shape local sweep of all 40 served tools against live upstream 0.11.0 from the 0.7.0 cache

- Record: `apps/oak-curriculum-mcp-streamable-http/docs/uat-reports/2026-09-02-local.md`
  (407 scripted `tools/call`s, every enum value and every optional/combined/negative
  shape, each anomaly re-checked directly against upstream with the same key).
- THE FINDING the 2026-09-02 spec-drift census predicted as a "composes with the
  pagination lane" delta is a live P1 today: upstream 0.11.0 put `limit`/`offset`
  (default 20) on `/key-stages/{ks}/subject/{s}/units` and `.../assets`; the 0.7.0-generated
  tools send no limit, get page one, drop the Link header (not classified paginated) and
  return with NO pagination echo. KS2 maths units: local 20/88 (1 of 4 year groups); KS3
  english: 20/25 with the SAME year-group count — invisible truncation. Cure is the MCP-630
  refresh (regen marks them paginated; the echo on feat/mcp-pagination-echo then covers them);
  sequence: land the echo, then refresh.
- Local P2 found by bracketing, not by reading: `search` schema says size<=100, RRF
  `rank_window_size` is 80, so 81..100 fail with an ES validation error on every RRF scope
  (suggest exempt). A bound declared at one boundary and enforced at another drifts; bracket
  a declared ceiling live before trusting it.
- Instrument lessons: (1) `dev:observe:noauth` refuses to boot without ES keys despite the
  MCP `.env.local` comment saying the client is lazy — env validation is strict; the search
  CLI's `.env.local` holds them, inject via process env without echoing. (2) Validation
  errors are `isError` tool results carrying "MCP error -32602 …", not JSON-RPC errors — my
  matrix expected rpc-error for 56 rows and the runbook §12.2 says the same wrong thing;
  the harness's expect column must encode the observed contract, not the runbook's.
  (3) `curl -D $SP/h.txt` with an unset `$SP` silently produced eleven `000` results —
  an absolute scratch path in a fresh shell, again; the persisted-output cap hid the
  matrix log twice and Read on the persisted file was the cure.
- Claim opening refused without a live comms watcher (F-95) even for a doc-only session;
  armed `comms watch` with `--supervisor-pid` = the dev server pid so it dies with the server.
