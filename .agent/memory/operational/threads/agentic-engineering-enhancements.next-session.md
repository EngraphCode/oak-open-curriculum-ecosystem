...
requirements ratified. The assumptions-expert verdict (ratify-with-revisions, 13-item revision
queue, 4 restart questions) is conserved IN the record's §Review; the architecture review did
not complete — its full re-dispatch scope (A–F) is also in §Review; no partial verdict was
folded. **Restart sequence:** (1) re-dispatch architecture-expert-wilma per §Review scope +
re-run assumptions-expert only if the revisions materially reshape; (2) work the revision queue
(criticals first: restore the recall-heavy-miners owner sharpening; de-confound the blindness
figure; the token-audit deferral needs OWNER acknowledgement; salvage ws3–ws5 routing explicit);
(3) land the one atomic landing set — the PDR-122 invariant-2 amendment (keep amendment points
1 and 3 plus the measured-n_eff clause in the PDR; remedy-ladder technique names stay in the
record/plan, per reviewer observation 10), the matching touch on
`agentic-judgment-conserve-by-default` lines 27–28, the plan promotion `future/`→`current/`
with executable P1 workstreams absorbing salvage ws3–ws5, the salvage plan's truthful close,
the README rows, and the record's tense flip to landed. Build-vs-Buy
verdict already taken (build; MLflow/Braintrust-class platforms named and declined — SaaS/Python
stack vs a small typed in-repo registry). WS2/WS4 of ADR-200 proceed independently and must NOT
wait.
...
| `Hedgehog stirs Rime` | `claude-code` | `claude-fable-5` | `da727a` | `phase-0 corpus-generalisation design-ratification, PAUSED owner-directed mid-review-absorption: design record drafted + committed with all (a)-(e) + 17-question verdicts + D7-D9; assumptions-expert verdict (ratify-with-revisions) conserved in the record §Review; architecture review to re-dispatch; landing set (PDR-122 amendment + plan promotion + salvage routing) = restart work per the section brief above` | 2026-07-05 | 2026-07-05 |
