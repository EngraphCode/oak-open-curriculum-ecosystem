...
`120 × 5 × ~50k ≈ 30M` would **TRIP a 16M ceiling** (16M was the RETIRED combined template's default; the
SPLIT templates carry NO default — `__VALIDATE_TOKEN_CEILING__` is filled from scratch).
`launch-preflight-and-cost-reconciliation` MUST set `VALIDATE_TOKEN_CEILING` to ~25–30M (worst-case =
candidateCount × MAX_VOTERS_PER_CANDIDATE(=5) × ~50k) and/or moderate the candidate count. Run the full
...
`agent-tools/src/corpus-analysis/` (88 unit tests, green under all gates). The graduate
