...
1. **The full-processing pass — ONLY ws1d BATCH E-b8 REMAINS (everything else DONE:
   ws1c/e/f/g + E-b1 2026-07-03, Gust hunts Headwind, `6708fb8eb`→`d79b56c63`; E-b2..E-b7
   2026-07-04, Otter hunts Jetty, `de001662d` + the ripgrep clustered-replace hook-policy
   fingerprint `b6af3e0bd` — the six-instance fires-despite-home recurrence graduated to
   action-time enforcement — + `1d32ff917` + `fb29ff2ef` + `558f046af` + `0fd8e57a7` +
   `f0b754f9c` + the E-b7 commit).**
   Tier E ranks 176–187 (12 items, one commit; jq: `.tierE[175:187]` of
   `.agent/reports/agentic-engineering/large-corpus-analysis-tooling/data/discovery-run-salvage-tiers-2026-07-02.json`
   for E-b8; when it lands, ws1d completes and the mandate's set-level acceptance — the
   inventory re-run — fires). The plan's ws1d todo carries drain state and the worked shape; the ws1g
   ...
   (`precedence-is-not-approval`). Fresh-seat opener (the plan's ws1d todo owns the pickup):
