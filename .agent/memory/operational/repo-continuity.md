...
| `oak-slack-assistants` | Internal-use agentic Slack assistants over Oak's MCPs on one shared `slack-assistant` framework: **Ask Oisín** (M1 — project/repo navigator via GitHub MCP) + future **Ask Oak** (curriculum, Oak MCP). Design verified against primary vendor docs + the live Oak MCP; plan through 3 review rounds (defect / open-question / round-2, each fleet + adversarial verify) + coherence reconcile; **v1 = internal POC**; plan **READY for WS0–WS8**, WS9+ gated on owner-handled provisioning. On PR #328 (open). Next: WS0 ADR (apps/slack tier + framework adapter-tier + eslint boundary config) then scaffold | [record](threads/oak-slack-assistants.next-session.md) | claude-code / claude-fable-5 / Gale guards Eyrie (`33f49e`) / design-verify + plan-author + 3 review rounds + coherence (docs only, no code) / 2026-07-08 — successor: Copper (`48382d`) |
