...
- **WS-B explain MCP surface — RESUMED + test-surface refactored (2026-06-26, Skipper tracks Kelp).**
  Owner-directed continuation of the `orientation-skills-family` WS-B lane. The 8 D1+D2 commits
  (explain effort-orientation MCP surface) were cherry-picked onto a fresh worktree off `main` (branch
  `worktree-ws-b-explain`) and the test surface **refactored to behaviour-only** (commit `03c279ca2`,
  +114/−605): the two fingerprint drift-guards + the content-grep tests DELETED as config-pins, the
  firewall made a **PR-review property, not a test** (owner test-doctrine: tests prove behaviour, never
  configuration or content). **Remaining (owner scope):** D0 audience-model audit + consistency + add
  data-analyst/data-scientist (effort-domain, firewalled from curriculum); D3 tool / D4 prompt / D5
  value-proxy; PR to main. Ship-gate live (no user exposure until the skill serves the new expert
  audiences + the `DATA-SOURCES.md` governance decision). Full handoff: the thread record §WS-B MCP
  surface. **`.agent/` files on this branch need a SEMANTIC merge (`/oak-semantic-merge`) at branch→main,
  never a git line-merge.** Branch pushed (owner-authorised).
- **Worktrees handed to Alder tracks Topsoil (2026-06-26):** `oak-pr-watch` (proto-chain dispatch guard
  `2fe5909c2`) + `oak-pilot-ws-e` (search-cli cli-contract e2e `9e2b49b7e`) to evaluate + integrate;
  Alder ACKed + verified first-hand (comms `7adf6f80` + ACK). The other ~7 stale worktree branches are
  squash-merged leftovers of PRs #222–#225 — curator-pass cleanup, no unmerged substance.
