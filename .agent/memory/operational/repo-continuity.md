...
| `oak-slack-assistants` | Internal-use agentic Slack assistants over Oak's MCPs on one shared `slack-assistant` framework: **Ask Oisín** (M1 — project/repo navigator via GitHub MCP) + future **Ask Oak** (curriculum, Oak MCP). Design verified against primary vendor docs + the live Oak MCP; plan through 3 review rounds (defect / open-question / round-2, each fleet + adversarial verify) + coherence reconcile; **v1 = internal POC**; whole estate **READY FOR REVIEW** (2026-07-08 — logging/observability approach reopened), WS9+ gated on owner-handled provisioning. On PR #328 (open). Next: a **review round** — vendor-verify the Next.js telemetry topologies, populate the cost/value theory, owner back-end-only-egress decision, then revise WS0/WS8 (framework imports NO vendor provider; providers compose at the app root, NOT `sentry-node`) + fix `@oaknational/logger`'s `node:crypto` portability leak + its missing browser-safety test. Detail: the [logging design record](../../research/outreach/slack-assistant-logging-observability-design.md) + thread record | [record](threads/oak-slack-assistants.next-session.md) | claude / claude-opus-4-8 / Kiln wakes Copper (`48382d`) / successor — logging re-exploration + ready-for-review handoff (docs only, no code) / 2026-07-08 ← claude-code / claude-fable-5 / Gale guards Eyrie (`33f49e`) / design + plan + 3 review rounds + coherence / 2026-07-08 |
...
0a. **Deep consolidation status: due — LIVE AND UNGATED (owner ruling: the dedicated pass
   runs after R0c merges; R0c merged 2026-07-08, PR #325). Scope at 2026-07-08 close:
   napkin ~700 lines carrying five unprocessed seat-windows (Stoat, Leopard, Goshawk,
   Rigel, Pelican), two resonance practice-box incoming files, three pending-graduations
   items (quiet-pipe class graduation-ripe at 5+ instances). Not run at the Pelican
   closeout (dedicated-pass scale; owner arranging a successor); the pass also trues this
   note.** Prior: bounded piece 2026-07-07 (Goshawk) — `standby-runway-handoff` graduated
   to `patterns/`. Prior dedicated pass: **EXECUTED 2026-07-06 (Zenith wakes Perigee, dedicated pass,
