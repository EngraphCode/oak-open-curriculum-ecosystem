...
| `strategy-and-plan-estate-holistic-review` | **REFRAMED 2026-06-22 ([ADR-200](../../../docs/architecture/architectural-decisions/200-intent-as-a-living-idea-graph.md)): planning-estate REWRITE on a living idea-graph.** Ideas are the fundamental node; the graph is the machine-readable SSOT (JSON + JSON Schema on `graph-core`); documents are the co-equal human-navigable embodiment via frontmatter edges. The prior survey/Pass framing is SUPERSEDED. ADR-200 + ADR-201 (Proposed) + plans COMMITTED; progression GO. **CORPUS REFOUNDING INSERTED (owner-directed 2026-07-06):** losslessly re-found the plan corpus around intent FIRST (the donor-estate method at oak scale) as the independently valuable intermediate step; controlling protocol plan [`plan-corpus-refounding.plan.md`](../../plans/product-development-governance/current/plan-corpus-refounding.plan.md) (P1–P14; placed judgement; zero-judgement workers; owner-gate register) + design record + dated `planning-estate-rewrite` amendment (`ws-r-corpus-refounding` gates ws6; WS6 substrate = refounded corpus + frozen archive via binding-clause provenance). Cross-estate designed with the resonance exchange seat; donor review sound-with-revisions, zero overturns, absorbed. **R0a COMPLETE ON MAIN (PR #321 merged `af6c285e5`, 2026-07-07)** — the full mechanical instrument (freeze, verify-freeze, inventory, residue, sweep, plant-orphan, plant-challenge-canary, merge-recheck, tile, default-ledger, claim-census, batch-status) is landed with its discrimination proofs. **G1 DISCHARGED (in-chat owner sitting 2026-07-07; record = the G1 packet §9; PR #322)**: freeze rule ratified (clean v1; destination `.agent/refounded-plans/` with terminal archive+rename; EMPTY sanctioned set; reader-sample cure at 10%); Walk-A derivation input recorded (few thread-level plans + implementation collections + holding bucket; zero concepts lost); WS6 HARVEST scope strengthened (ALL live + ALL archived plans in full). **Next: R0b** (plan-state engine; Stoat's §3 contracts binding); S0 gated on G2+G3 only. Pickup = the [Goshawk handoff record](../../state/collaboration/handoffs/2026-07-07-goshawk-g1-discharged-and-r0b-runway.md) on retained claim `7b10679e`. **WS2** idea-node schema + id-minting proceeds in PARALLEL unchanged; **WS4 thin-slice-proof stays a hard gate** before the harvest (WS6). V0 = the form new plans take. Read ADR-200 + the thread record first. | [record](threads/strategy-and-plan-estate-holistic-review.next-session.md) | claude-code / claude-fable-5 / Goshawk calls Sundog / R0 successor #3 — R0a merged + G1 discharged + closeout handoff / 2026-07-07 ← Wildfire herds Sulphur / corpus-refounding commission → cross-estate protocol design → landing / 2026-07-06 ← Perseus lifts Umbra / 2026-06-23 (prior seats: full table in thread record) |
...
0a. **Deep consolidation status: completed this handoff (2026-07-07, Goshawk calls Sundog) —
