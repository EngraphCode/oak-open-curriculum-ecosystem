...
| `strategy-and-plan-estate-holistic-review` | **REFRAMED 2026-06-22 ([ADR-200](../../../docs/architecture/architectural-decisions/200-intent-as-a-living-idea-graph.md)): planning-estate REWRITE on a living idea-graph.** Ideas are the fundamental node; the graph is the machine-readable SSOT (JSON + JSON Schema on `graph-core`); documents are the co-equal human-navigable embodiment via frontmatter edges. The prior survey/Pass framing is SUPERSEDED. ADR-200 + ADR-201 (Proposed) + plans COMMITTED; progression GO. **CORPUS REFOUNDING INSERTED (owner-directed 2026-07-06):** losslessly re-found the plan corpus around intent FIRST (the donor-estate method at oak scale) as the independently valuable intermediate step; controlling protocol plan [`plan-corpus-refounding.plan.md`](../../plans/product-development-governance/current/plan-corpus-refounding.plan.md) (P1–P14; placed judgement; zero-judgement workers; owner-gate register) + design record + dated `planning-estate-rewrite` amendment (`ws-r-corpus-refounding` gates ws6; WS6 substrate = refounded corpus + frozen archive via binding-clause provenance). Cross-estate designed with the resonance exchange seat; donor review sound-with-revisions, zero overturns, absorbed. **R0a COMPLETE ON MAIN (PR #321, `af6c285e5`); G1 DISCHARGED (PR #322). R0b COMPLETE ON MAIN (PR #323 owner-merged `751ce9699`, release 1.63.0, 2026-07-08)** — the plan-state recomputation engine (one engine, two adapters: permanent gate over V0/V0.1 frontmatter + disposable audit over census records; `plan-state` CLI; status-mapping table v1 with OG-2 ratification PENDING and a mechanical audit-mode refusal until it flips) plus shared hardenings (dangling-symlink write refusal; terminal control-char strip). **Next: R0c** (owner-gate register incl. the OG-2 row + cost ledger; first truing = the `r0b-plan-state-tool` todo), then r1 gated on G2+G3 (+OG-2 for audit mode). Pickup = the [Rigel handoff record](../../state/collaboration/handoffs/2026-07-08-rigel-r0b-merged-and-r0c-runway.md) on retained claim `7b10679e`. **WS2** idea-node schema + id-minting proceeds in PARALLEL unchanged; **WS4 thin-slice-proof stays a hard gate** before the harvest (WS6). V0 = the form new plans take. Read ADR-200 + the thread record first. | [record](threads/strategy-and-plan-estate-holistic-review.next-session.md) | claude-code / claude-fable-5 / Rigel turns Void / R0 successor #4 — R0b built + merged (PR #323) + closeout handoff / 2026-07-08 ← Goshawk calls Sundog / R0 successor #3 — R0a merged + G1 discharged / 2026-07-07 ← Wildfire herds Sulphur / corpus-refounding commission → cross-estate protocol design → landing / 2026-07-06 ← Perseus lifts Umbra / 2026-06-23 (prior seats: full table in thread record) |
