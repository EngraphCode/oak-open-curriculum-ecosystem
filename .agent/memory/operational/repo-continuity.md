...
- **Curriculum Hub — MULTI-SESSION PROGRAM, LIVE on `feat/curriculum-hub-demo` (owner-reframed 2026-07-01).** Reproduce the ENTIRE Oak Curriculum Hub from the **Claude Design canonical export** (`demos/curriculum-hub-hw/claude-design-canonical-export`) — ALL pages + ALL components, visual-matched — plus curriculum-search integrations (two search functions: live ES curriculum + local search over the bundled export data). Full reproduction, **no stubs, no hedging**. Standing owner principle (any Claude Design project, forever): always visual-match + all pages/components (user-memory `claude-design-always-full-reproduction`). Strict-everywhere + long-term architectural excellence is the owner's explicit bar. **Director chain:** Herring #1 → Swordfish #2 → Lantern #3 → Hawthorn #4 → Sycamore #5 → Panther #6 → Birch #7 → **Comet hunts Lightyear #8 (ACTIVE, 2026-07-02)**. All transfers clean PDR-064 two-moments; the 2026-07-01→02 arc (four owner ratifications, 5 gated commits, three live lanes) is in the thread record §CURRENT STATE + `handoffs/2026-07-02-curriculum-hub-director-birch.md`. **Cast (owner-launched):** styling Dolphin→Laurel→Kite→Linnet→**Typhoon turns Aether** (cf62bda9); data Frigate→Polaris→Eclipse→Cinder→**Deneb mends Perigee** (fd0ee59e); all rotations clean PDR-063. **Current state (pre-Course-assembly COMMITTED; main #291 MERGED 39a3aaf50 through the full gate; branch PUSHED; Course-assembly in progress; team PAUSED for owner restart):** Standards page (browse+detail) DONE + **§E SIGNED OFF**; Course generator + 214-block compile-time-validated module DONE; hub-search local dispatch DONE; 18 block components + exhaustive BlockRenderer DONE; Block union extended schema-first; eslint tooling-zoning ratified (config-expert PASS). Reviewer passes (type/react/a11y/test/config) clean on all landed surfaces. **NEXT:** Cinder → GATING visual-target render (timeboxed, headless-blank risk); Linnet → Hub landing (wire searchHub + 6 cards) then remaining pages (Course page, Framework, Rubrics, Exemplars/Wiki, Lesson view); Hawthorn → settle content-tree type interface at Course-assembly + clear tracked a11y follow-ups (block #5-7/#9 + 2 detail + globals.css reduced-motion + jest-axe) before final §E + run the milestone commit once §A–I green. **Pointers:** guiding plan `.agent/plans/curriculum-hub-demo/active/port-prototype-to-live-demo.md` (DoD §A–I); **thread record + Active-threads row LIVE** (`threads/curriculum-hub-demo.next-session.md` — full detail); Director pickup `handoffs/2026-07-01-curriculum-hub-director-hawthorn.md` (→Sycamore, front-loaded); research `.agent/research/claude-design-integration.md`. Branch PUSHED to origin (owner, 2026-07-01); DoD now §A–J (§J = Vercel web-deploy to show people); licence DISSOLVED (root LICENCE MIT + LICENCE-DATA OGL; assets committable). **Two system defects still to graduate (NOT demo-local):** (a) `@oaknational/eslint-plugin-standards` configs.react/next crash under ESLint 10; (b) workspace SDKs' `development→src` export condition unconsumable by Turbopack dev.
...
| `curriculum-hub-demo` | Reproduce the ENTIRE Oak Curriculum Hub from the Claude Design canonical export — all pages + all components, visual-matched — + two-search (live ES + local) + 6 destination cards, to the plan **DoD §A–I**. Spine (exhaustive BlockRenderer + 18 block components, AA-fixed) + Standards data-view + training-course local search VERIFIED green; Standards page (Kite) + Course generator/hub-search (Eclipse) in flight. Branch `feat/curriculum-hub-demo` PUSHED to origin; DoD §A–J (§J web-deploy). main #291 merged (39a3aaf50); Course-assembly in progress; team PAUSED for owner restart | [record](threads/curriculum-hub-demo.next-session.md) | claude / claude-fable-5 / Comet hunts Lightyear / director #8 / 2026-07-02 (live implementers: Galago turns Footfall styling `cf62bda9`, Limpet herds Marsh data `fd0ee59e`, Thyme guards Dewfall hygiene `16be897b`; Director chain Herring→…→Panther→Birch→Comet; full cast + lane state in thread record) |
...
0. **Deep consolidation status: due — napkin over-limit (rotation), the pending-graduations
   register carries `due` items (director-operating-model + the sampled-data pattern with its
   fired second-instance trigger), and the coordination-tier comms curator-pass backlog stands.
   Not well-bounded for the 2026-07-02 Birch closeout (three implementer lanes mid-drive under
   Director Comet); a dedicated fresh-context pass picks it up.** (Set per session-handoff step 9,
   Birch mends Petal, 2026-07-02.)
