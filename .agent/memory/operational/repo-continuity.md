...
| `cloud-environment-bootstrap` | Shared claude.ai "Practice Repos" cloud environment: fail-fast discovery-driven setup script + validation harness (preflight probe, phase-banner instrumentation, diagnosis protocol) — artefacts identical in both Practice repos under `.agent/claude-harness-integrations/`. **ENVIRONMENT FIXED 2026-08-24**: the outage's root cause was the discovery find pipeline exiting non-zero (no `/workspace` on the builder) under `set -euo pipefail` — present since the script's first paste; fixed by tolerating the pipeline exit, fresh build owner-confirmed. Diagnosis loop proven end-to-end (preflight 12/12 exonerated all hosts; the card named the dying line); fragile-hosts register emptied by evidence. ARC CLOSED: all PRs merged 2026-08-24 (castr #47 12:54Z, OCE #12 13:03Z); retrospective landed via OCE PR #13 with four routed proposals awaiting the consolidation pass. Thread quiescent; full record + review-loop lesson: thread record | [record](threads/cloud-environment-bootstrap.next-session.md) | claude-code (cloud) / claude-fable-5 / Buzzard weaves Airstream (01e90b) / environment repair + harness author / 2026-08-24 |
