...
- **Curriculum Hub demo wired to live data — LIVE, near-done, on `feat/curriculum-hub-demo` (2026-06-30, n=2: Titan weaves Ether + Herring holds Jetty).** Heather W's demo (`demos/curriculum-hub-hw/oak-curriculum-hub`, untracked) re-platformed onto the live Oak search + content SDKs. Data-plane seams (Result over throw, no shims), config, and Tailwind v4 styling all DONE and integrated; demo's own gates GREEN (type-check / full-strict lint / `next build` / `pnpm dev` + live data). **NEXT: run repo-wide `pnpm check` (single gate-runner; live `.env` in the demo dir) → owner commit go-ahead → commit by explicit pathspec.** `demos/` is a prototype-zone (owner-directed): exempt from repo-wide knip/format/markdownlint only, but holds its own full-strict ESLint. **Two system defects to graduate (NOT demo-local):** (a) `@oaknational/eslint-plugin-standards` configs.react/next crash under ESLint 10 (eslint-plugin-react@7.37.5) — fix in `packages/core/oak-eslint`; (b) workspace SDKs' `development→src` export condition unconsumable by Turbopack dev — repo decision needed. Full detail + grounded SDK shapes in `napkin.md` (2026-06-30 entry).
