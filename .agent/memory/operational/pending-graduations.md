...
- **The agentic deterministic substrate (LLM judges atomically; code aggregates; that code is gated agent-tools TS)** `[captured: 2026-06-29 | source: large-corpus-analysis-runbook-v2-design-2026-06-29.md + the v1 Discovery recall-arithmetic defect (meta agent reported 0.72; its own per-item judgments summed to 0.56/0.28) + owner directive 2026-06-29 | target: a new PDR (Practice substance) with three clauses — (1) an LLM emits only atomic per-item judgments; (2) every count / fraction / threshold / verdict / route is deterministic code; (3) repeatedly-used agent deterministic code lives in the agent-tools workspace as proper TypeScript under all repo gates (ESLint / type-check / vitest-TDD / Result pattern / schema-first), never inline-in-workflow-scripts or throwaway | trigger: the v2 implementation lands the aggregation module in agent-tools (first worked instance), OR a second independent instance | size: M | status: GRADUATED 2026-06-30 → PDR-122-agentic-judgment-pipelines.md (the three clauses + the conserve-by-default correction)]`
- **Agentic design-panel protocol + its iterative self-improvement loop** `[captured: 2026-06-29 | source: agentic-design-panel-protocol-2026-06-29.md (first worked instance: the corpus-analysis v2 design panel) | target: a PDR (Practice substance) and/or a PDR-120 reference runbook — the reusable fan-out-generation → diverse-lens-critic-ensemble → orchestrator-synthesis protocol, INCLUDING its self-improving property (each application yields both the design AND a refined protocol-for-running-panels) | trigger: applied + refined on a SECOND design elsewhere (owner-directed: apply + refine in other places) | size: M | status: pending]`

- **agent-tools workspace needs explicit architectural conventions (invocation, error-handling, dependency, gate-wiring)** `[captured: 2026-06-29 | source: agent-tools-architecture-state-and-check-encoding-handoff-2026-06-29.md + agent-tools-encoding-guard-and-architecture-2026-06-29.md + owner direction 2026-06-29 ("a real problem, a lack of architectural direction on agent-tools") | target: an ADR (this-repo architecture) consolidating ADR-178/168/041/159 — source(tsx)-vs-built(node-dist) execution, error-handling (Result vs exit-codes-at-boundary), permitted workspace deps, gate-wiring/ordering, and pure-mechanism-in-core vs Oak-policy-in-agent-tools; possibly a source-condition export so tooling resolves workspace deps build-free. The ADR is WS1 of the now-authored strategic plan future/agent-tools-architecture-standard.plan.md | trigger: that plan's promotion to current/ (its WS0 execution-model decision pass), OR the next build-order/invocation-inconsistency friction | size: L | status: pending]`
