...
| Structural firing points for twice-recurred failure classes (retrospective 2026-08-24 proposal 4): when a failure class recurs with its cure already existing as prose (worked: the no-tally arcs PR #390, PR #570, and the 2026-08-24 harness arc), the consolidation pass routes the cure to a structural firing point — a hook, validator, or contract entry step — or records why not. Provenance: `.agent/reports/agentic-engineering/why-the-outage-outlived-its-six-character-fix-2026-08-24.md` §Proposals | Measurement begins when proposal 2's tally-at-PR-open entry contract lands (the enabling FAST change), not at capture: from that landing, no review arc exceeds six settled rounds without a tally-backed step-back firing, decidable only once at least one qualifying arc (a bot-reviewed PR reaching a settled round) has run. If the contract has not landed or no qualifying arc has run by review, the review EXTENDS rather than promotes or kills | The prediction fails, or structural firing points accrete as ceremony that never fires — kill this row into that finding | 2026-10-24 |
