...
todos:
  ...
  - id: ws-r-corpus-refounding
    ...
    status: pending
  - id: ws6-deep-harvest
    content: "GATED on ws4 + ws3 + ws-r-corpus-refounding: deep harvest of the REFOUNDED plan corpus (count re-derived at harvest, NOT frozen) PLUS the frozen pre-refounding archive via binding-clause provenance edges (the owner-ratified substrate, ADR-200 §Consequences Amendment; STRENGTHENED by owner ruling 2026-07-07, G1 packet §9: the harvest reads ALL live plans AND ALL archived plans IN FULL — achieved concepts are as first-class as intended concepts, so the refounding's archive-as-sweep economy does NOT narrow the harvest) PLUS VISION.md + docs/strategy/ (owner-ratified 2026-06-22) to the finalised schema -> the preserved idea-graph spanning all altitudes (the existing thin Pass-1 idea data is re-derived/enriched, not relied on). Acceptance: every in-scope doc harvested — including the full archive; every idea has provenance + class; coverage logged; the graph validates against the schema; vocabulary-friction logged as a first-class output (forced-misfit ideas, missing facets/domains/edges, scale-misfit) -> feeds ws6b."
    status: pending
