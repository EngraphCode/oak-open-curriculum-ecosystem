...
todos:
  ...
  - id: ws1-merge-readiness-checker
    content: "WS1: a mechanical merge-readiness checker in agent-tools — `pnpm agent-tools:pr-merge-readiness <pr>` (exact CLI name TBD at build). Given a PR number it reports and exits non-zero unless ALL hold: (a) the PR is POSITIVELY mergeable — GraphQL `mergeable === MERGEABLE` (not merely `!== CONFLICTING`, which wrongly passes `UNKNOWN`); since GitHub computes mergeability asynchronously, treat `UNKNOWN` as not-yet-ready and poll/retry a few times before failing, rather than passing it; (b) every required check is terminal and green (no pending/failed); (c) every review thread is resolved (GraphQL `reviewThreads.isResolved`) OR carries a reply (from any author — a human or another author may refute or resolve a comment, not only the merging agent; this matches the Risks clause); AND (d) readiness binds to the pr-lifecycle skill's review-round state machine (`.agent/skills/pr-lifecycle/SKILL-CANONICAL.md`), NOT green-checks-and-resolved-threads alone: the expected reviewer legs (item 3), the round SETTLED for the current tip (item 4), a zero body-finding tally across threads AND review bodies (item 2), and late-harvest handling of out-of-order/older-tip reviews — bound BY REFERENCE, the skill holding the single definition and this checker consuming it, never restating the mechanics here. REST `/pulls/{n}/comments` does NOT carry thread resolved-state — use the GraphQL `reviewThreads` connection. TDD; pure core (verdict function over fetched thread/check state) + thin gh/GraphQL I/O adapter; no global state. Output names each blocking thread/check so the agent knows exactly what to resolve."
    status: pending
    depends_on: []
  - id: ws2-oak-pr-skill
