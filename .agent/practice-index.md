...
| [PDR-135](practice-core/decision-records/PDR-135-quality-gates-are-a-registered-corpus.md) — quality gates are a registered lever corpus (registration is the membership condition and an unregistered gate is the defect, not the register's; derived facts recomputed rather than maintained, bidirectional drift failing; a gate may report only what it verified — verified-pass, verified-fail and could-not-check never collapsed; registration records reachability at every binding grain, not invocation alone; three derivability tiers; Proposed) | Phenotype ADR **pending** — step 3 of the ratified `quality-gate-ledger` delivery node (MCP-491; named without linking, since the plan node has not yet reached this branch's base), which carries the ledger artefact and its schema, the recomputing validator, and the supersession of the existing coverage matrix by a generated view |
