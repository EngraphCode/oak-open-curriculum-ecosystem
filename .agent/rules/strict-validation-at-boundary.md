...
An admission predicate binds every axis it reasons about. An allowlist entry
is a (name, kind) pair, never a name alone: a transient-file allowlist that
exempted by basename before kind handling let a symlink wearing a transient
name ride out of the symlink refusals, and a place-only design-token
admission did the same in the same sitting (2026-08-19). Validate each axis
the decision depends on, at the boundary the value crosses.
