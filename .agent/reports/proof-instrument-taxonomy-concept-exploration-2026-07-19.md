...
| --- | --- | --- |
| Type anchors (bidirectional, negative, exhaustiveness) | `packages/libs/logger/tests/logger-contract.typecheck.ts` (`Assert`/`AssertFalse`, off-API, tsc-proven) | Module-scope `expectTypeOf` in a runtime test file (plan-state — migrates per proposal 2's positional router rule); public-API export with zero consumers; dummy consuming `it()`; type-only `it()` blocks across three files (banned shape, live; exemplars in the appendix) |
