...
| Collaboration-State Surface Restructure | 2026-06-27 | Applied the collaboration-state placement contract to the surface family (`collaboration-state-lifecycle.md`, `collaboration-state-conventions.md`, the `agent-collaboration` directive, and the four `.agent/state/collaboration/` schemas): substance moved to canonical homes, vocabulary normalised, cross-references verified. Phase-4 acceptance met; the §6 empirical-validation clause (the contract must have "prevented at least one wrong-file landing in a subsequent graduation") **waived as an unobservable future-prediction** — bounded to this shape, not a general licence to waive empirical self-gates. Contract home: `.agent/memory/executive/collaboration-state-placement-contract.md`. (D3, agent-operability rationalisation.) | [archived plan](agentic-engineering-enhancements/archive/completed/collaboration-state-surface-restructure.plan.md) |
