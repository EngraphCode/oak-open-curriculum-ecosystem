...
todos:
  ...
  - id: pr3-cycle-four-theme-gate
    content: "PR3 cycle 3: extend buildContrastReports from light/dark to all four theme trees, composing light base ⊕ overlay per theme BEFORE resolution (ADR-213 §2 amendment — a sparse overlay resolved alone spuriously reports unresolved tokens) and excluding alpha-literal and expression paths from the resolved hex map; run the design system's contrast manifest; surface the AA-vs-AAA high-contrast gate level as the owner gate; re-baseline design-token-practice.md's two-theme wording in the same PR. Still open as of 2026-07-20: PR #412 landed the composition semantics (ADR-213 §2 amendment) and the core validators this cycle composes, but buildContrastReports still validates light/dark only and design-token-practice.md still carries the two-theme wording."
    status: pending
