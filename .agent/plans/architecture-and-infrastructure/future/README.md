...
| P2 | [test-purity-quick-wins-execution.plan.md](./test-purity-quick-wins-execution.plan.md) | Owner-commissioned test-purity quick-wins set (suffix invariants, vitest config purity, CI relocation) rescued 2026-07-20 from the private sibling Resonance estate; promotion trigger = the adaptation pass re-deriving every source-estate fact against this tree |
...
| P1 | [graph-corpus-data-blob-storage-and-git-lfs.plan.md](./graph-corpus-data-blob-storage-and-git-lfs.plan.md) | **Owner-flagged high priority (2026-07-06).** The 26 MB generated `graph-corpus/data.json` is a legitimate cross-workspace runtime asset but is stored as a monolithic minified JSON re-committed wholesale each regen (~116 MB history for one file). Evaluate gzip / **Git LFS** / boundary-separation / history surgery; includes the required CJS→ESM loader fix. Symptom already mitigated by PR #306 (pushed-range secret scan); this is the root. |
