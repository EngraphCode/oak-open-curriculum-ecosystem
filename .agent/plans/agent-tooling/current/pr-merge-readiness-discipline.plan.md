...
todos:
  ...
  - id: ws6-pr-watch-compound-floor
    content: "WS6: extend `pnpm agent-tools:pr-watch` to surface the compound-read floor it currently omits — review bodies AND per-reviewer latest-review tip-binding — so the Phase 7 round-owed / reviewer-leg gate becomes machine-checkable rather than depending on the manual Phase 3 GraphQL harvest on every wake. Today pr-watch reads only PR-view fields, REST review comments, and thread counts, so a summary-only review carrying findings in its body does NOT change its snapshot and an OWED reviewer leg is invisible to it. Add the `latestReviews(first:20){pageInfo{hasNextPage endCursor} nodes{author{login} commit{oid} state submittedAt body}}` connection (the pr-lifecycle skill's state-machine item 1 selection), retaining each review's `commit.oid` to bind the reviewer's latest review to the current tip and paging past 20 when `totalCount` exceeds the window. Bounded `reviews(last:20)` is the WRONG selection here — a long review history pushes an earlier bot's latest review out of the window — so use `latestReviews` with pagination. Emit review-body arrivals and per-reviewer tip-binding as watcher state changes."
    status: pending
    depends_on: [ws1-merge-readiness-checker]
isProject: false
---
