...
overview: >-
  Give agents a distinct Linear identity — an OAuth2 application installed
  as an agent (actor=app) — so agent activity in Linear is attributed to
  the bot, not the owner, mirroring the jimbot-oakington-iii GitHub App
  arc. This plan is the owner-commissioned decision-complete deliverable:
  every choice carries a recommendation and rationale, the owner-side
  create-and-install runbook is sequenced, and implementation is cut as
  PR-shaped follow-on workstreams gated on owner ratification.
ticket: MCP-64
lineage:
  serves_thread: agentic-engineering-enhancements
  serves_stream: agent-tooling
  strategic_choice: identity-layer-attribution-over-content-markers
  derives_from:
    - .agent/plans/agent-tooling/future/github-surface-agent-identity.md
todos:
...
one NOT-READY round cured in place) — execution gated on owner
ratification + runbook (todo 0.3)
...
[`github-surface-agent-identity.md`](../future/github-surface-agent-identity.md))
