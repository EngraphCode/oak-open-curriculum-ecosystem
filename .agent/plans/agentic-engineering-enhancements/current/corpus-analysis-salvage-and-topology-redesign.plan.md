...
lane: current
status: "ws1 EXECUTED 2026-07-02 (Rosemary stirs Bracken): the corroboration cwd bug is fixed (18/18 corroborated), the salvage tooling is TDD-landed in agent-tools (salvage-driver + four modules), and the tier table + discovery report are committed — A=18 B=8 C=18 D=18 E=187 residual=0, tier C matching this plan's named ids exactly. ws1b (rescued-knowledge disposition) is DECISION-COMPLETE and executable now in a fresh seat; the owner's tier-E manual round is separate and owner-gated. Discovery-run rescue sets are now a canonical consolidation intake class (consolidate-docs §Discovery-Run Rescue Sets, 2026-07-03 — owner-directed on recurrence). ws2+ (topology redesign) still requires a fresh-session readiness review before build. Authored 2026-07-02 by Perseus wakes Oblivion at high context, post-run."
created: 2026-07-02
