...
| [napkin-corpus-discovery-run.plan.md](napkin-corpus-discovery-run.plan.md) | The napkin-corpus DISCOVERY RUN. The run EXECUTED 2026-07-01/02 (map 580 → reduce 246 → validate 26/220 → meta, checkpoints committed); the validate regime failed calibration, so the aftermath ran through the salvage plan (complete 2026-07-04); do NOT re-run the owner-gated action — it is spent and banked | CLOSED (2026-07-04) |
...
| [corpus-analysis-conservation.plan.md](corpus-analysis-conservation.plan.md) | Conserved the v2 corpus-analysis discoveries: WS-A (buffer graduated) + WS-B (the `agentic-judgment-conserve-by-default` rule) LANDED and stand; WS-C/WS-D (skill promotion + sweep) SUPERSEDED-WITH-ROUTING into the generalisation plan's Phase 0 (see the plan's Supersession Mapping appendix) | CLOSED (2026-07-04) |
| [corpus-analysis-salvage-and-topology-redesign.plan.md](corpus-analysis-salvage-and-topology-redesign.plan.md) | Salvage of the discovery run's paid-for output: full-processing COMPLETE 2026-07-04 (every stratum per-item dispositioned; tier E 187/187; set-level acceptance verified). Sole remainder: the ws2 topology readiness review, which folds into the generalisation plan's Phase 0 | FULL-PROCESSING COMPLETE; ws2 → Phase 0 |
