...
| [large-corpus-analysis-v3-extraction-grain.plan.md](large-corpus-analysis-v3-extraction-grain.plan.md) | The real, full Discovery scan of the napkin corpus: prompt-only map/reduce fix to preserve mechanism grain AND elicit longitudinal patterns, a cheap pre-spend probe, then a gated full run. Deliverable is conserved discovery (recurring + longitudinal); golden-baseline recall is the tuning means, not the end. Aggregation untouched. Successor to the DONE/SUPERSEDED `large-corpus-analysis-v2-implementation.plan.md` | REVIEWED — READY FOR EXECUTION (full run owner-gated) |
