...
| Later | [continuity-surface-drift-prevention.plan.md](continuity-surface-drift-prevention.plan.md) | Prevent `repo-continuity.md` / thread-record drift from compact-active-state into a session-landing log: (a) curate-not-append doctrine, (b) extend AX WS-4 drain-fix with a continuity drain-lag class, (c) derive the surface from the F-98 binding view (lens-resolved; routed into continuity-practice / AX / cost-of-collaboration) | (a) inline on next curation; (b) AX WS-4 reaching its failure-class cycle; (c) the F-98 binding-view build |
