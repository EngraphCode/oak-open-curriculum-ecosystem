...
| `OBSERVABILITY_SINKS`                  | `[]`                                                                                              | JSON-array sink list. Use `["sentry"]` for live Sentry; adding `posthog` validates and reserves the four PostHog values below at boot (capture begins when the runtime composition lands — MCP-241); production also requires a diagnostic sink (`sentry` or `file`) alongside it |
| `POSTHOG_PROJECT_API_KEY`              | —                                                                                                 | **Secret** (required when `posthog` selected): project ingestion key (`phc_`-class). Validated at bootstrap and reserved for the adapter (consumption lands with the runtime composition, MCP-241); stripped from handler-facing runtime config, never logged or serialised       |
