...
| 1.2 | OAuth metadata                         | `curl -sS ORIGIN/.well-known/oauth-protected-resource/mcp` (canonical host — the path-qualified route the handshake advertises) or `ORIGIN/.well-known/oauth-protected-resource` (alpha host, local)                                                                                           | HTTP `200`; JSON includes `resource` (the canonical `/mcp` URL) and `authorization_servers` naming the upstream authorisation server's issuer (`https://clerk.thenational.academy` on production) |
