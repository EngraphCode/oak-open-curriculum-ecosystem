 ...
 *   asset list resolves within this origin (the served-page asset posture:
 *   local design-system copy, local fonts, local artwork)
...
const LANDING_PAGE_CSP_DIRECTIVES = {
  ...
  /** Images: same-origin only — the logo and artwork are <img> references
   * into the app-served /oak-ds and /oak-assets copies, so no data: URI or
   * third-party host is needed. */
  imgSrc: ["'self'"],
