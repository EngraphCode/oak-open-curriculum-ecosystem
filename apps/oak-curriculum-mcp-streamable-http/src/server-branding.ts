 ...
 *
 * This file is the acorn geometry's home in this app: the path data was
 * originally extracted from the Oak-Web-Application sprite sheet
 * (`id="logo"`), and since MCP-368 the widget's `BrandBanner.tsx` renders
 * the wide WORDMARK from the design-system asset instead — the acorn mark
 * lives on only here, as the server icon. Two `data:image/svg+xml;base64`
 * URIs provide light-background and dark-background variants. Base64 is
 * used per the MCP spec recommendation for `data:` URIs. The SVGs are
 * ~2.4 KB each — well within the typical icon payload budget.
 *
 ...
 *
 * Shared between light and dark icon variants. Originally from the
 * Oak-Web-Application sprite sheet (`id="logo"`); this is the acorn's only
 * home in this app since the brand banner moved to the wide wordmark
 * (MCP-368).
 */
