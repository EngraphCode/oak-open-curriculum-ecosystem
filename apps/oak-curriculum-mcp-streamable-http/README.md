...
**Resources** (application-controlled) — `curriculum://model` (domain ontology, `priority: 1.0`), `curriculum://thread-progressions` (learning progression data, `priority: 0.5`), `curriculum://misconception-graph` (misconception data, `priority: 0.5`), and `eef://interpretation` (EEF evidence interpretation guide, `priority: 0.5`). All annotated with `audience: ["assistant"]`. The host application decides whether to inject these into the model's context. Clients that support resource auto-injection get orientation data without a tool call. Prior knowledge has no whole-corpus resource form — it is served by the anchored `get-prior-knowledge-graph` tool.
