...
| **Linear**                          | Issue tracking: issues, projects, cycles, comments in the Oak workspace    | Linear MCP plugin, enabled repo-wide in the tracked `.claude/settings.json` (Claude Code offers to install it on first launch); authenticate via `/mcp`  |
