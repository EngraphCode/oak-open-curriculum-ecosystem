...
**Amended**: 2026-07-13 — added the root `skills.md` entry point (Layer 3) for Linear coding sessions. A delegated Linear session runs Claude Code or Codex (inheriting those entry-point chains) and can use a repo-root `skills.md` as supplementary guidance, so the file carries the same thin-pointer shape as `AGENTS.md` (AGENT.md pointer plus rules-index line). Owner-directed.
