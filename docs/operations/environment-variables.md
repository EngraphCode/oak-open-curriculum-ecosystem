...
| `CANONICAL_HOST`                  | Address the server is served at behind an edge that overrides the Host; fixes every self-description URL, and the origin Clerk derives from the forwarded headers                                                                                   | Unset (self-describe per request)       | HTTP MCP server   |
