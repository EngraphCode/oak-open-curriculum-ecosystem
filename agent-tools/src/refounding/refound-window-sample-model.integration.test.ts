...
describe('buildWindowSample — expectation halts', () => {
  it('halts on a scanned-file count disagreement, naming both numbers', () => {
    ...
    expect(error.message).toContain('universe');
  });
  ...
  it('halts on a hit-line count disagreement', () => {
    ...
    expect(error.message).toContain('hit row(s)');
  });
  ...
  it('halts on a distinct-hit-file count disagreement', () => {
    ...
    expect(error.message).toContain('distinct hit file(s)');
  });
  ...
  it('halts when a hit file is outside the scanned universe', () => {
    ...
    expect(error.message).toContain('outside the scanned universe');
    expect(error.message).toContain('.agent/plans/in-class.md');
  });
  ...
  it("halts when a hit line lies beyond its file's line count at base", () => {
    ...
    expect(error.message).toContain('beyond');
    expect(error.message).toContain('.agent/prompts/a.md');
  });
...
describe('buildWindowSample — every-10th selection from index 0', () => {
  it('selects indices 0, 10, 20 of the sorted non-hit windows', () => {
    ...
    const value = unwrap(build({ files, expected: expectations(21, 0, 0) }));
    expect(value.sample.map((w) => w.file)).toEqual([
  ...
  it('selects exactly one window when fewer than 10 non-hit windows exist', () => {
    ...
    expect(value.universe.non_hit_windows).toBe(3);
    expect(value.sample.map((w) => w.file)).toEqual(['.agent/prompts/a.md']);
  });
  ...
  it('re-bases the stride on the non-hit list, not on all windows', () => {
    ...
    expect(value.sample.map((w) => `${w.file}:${String(w.window_index)}`)).toEqual([
  ...
  it('returns an empty sample when every window is hit', () => {
    ...
    expect(value.universe.hit_windows).toBe(1);
    expect(value.sample).toEqual([]);
  });
});
...
describe('buildWindowSample — the manifest carrier', () => {
  it('carries the schema version, base, window size, selection rule, and expectations used', () => {
    ...
    expect(value.window_lines).toBe(WINDOW_LINES);
    expect(value.selection_rule).toBe(SELECTION_RULE_V1);
    expect(value.expectations).toEqual({
      scanned_files: 1,
      hit_files: 0,
      hit_lines: 0,
    });
  ...
  it('round-trips through parseWindowSampleManifest (closed-shape contract)', () => {
    ...
    expect(parseWindowSampleManifest(value).ok).toBe(true);
  });
});
