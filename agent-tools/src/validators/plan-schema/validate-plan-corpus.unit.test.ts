...
describe('validatePlanFile — the D23 contract', () => {
  ...
  it('requires at least one ticket on a RATIFIED delivery plan', () => {
    const lines = replaceLine(
      replaceLine(
        replaceLine(
          replaceLine(
            dropLine(dropLine(DELIVERY_LINES, 'tickets:'), '  - MCP-101'),
            'status:',
            'status: ratified',
          ),
          'ratified_by:',
      ...
      'ratified_where:',
      'ratified_where: PR review',
    );
    const result = validatePlanFile('d.plan.md', planDoc(lines));
    if (isOk(result)) {
      expect.fail('should reject');
    }
    expect(result.error.messages.join('\n')).toContain('ticket');
