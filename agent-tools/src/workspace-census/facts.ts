 ...
 * topology at workspace grain (manifest-declared internal
 * dependencies), tracked-file profiles, and Oak-marker grep counts.
 * Facts carry no judgement: the judged pass reads them as evidence,
 * and the detector-facts / judged-readings separation the plan names
 * starts here.
 */
...
export function assembleFacts(input: AssembleFactsInput): SubjectFacts[] {
  ...
  return [...input.subjects]
    ...
    .map((subject) => {
      const manifest = manifestByDir.get(subject.dirPath) ?? null;
      return {
