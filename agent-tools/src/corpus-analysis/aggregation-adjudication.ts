...
function decidePreEnsemble(
  ...
  if (disposition === 'reroute' || isBorderline(tier0Outcome.verdict)) {
    return dispatchTier2From(0);
  }
...
/**
 * Tier 0: one voter. A kill is final on one (cheap, self-correcting). A reroute (base-rate-
 * only fail at high importance) escalates to the Tier-2 ensemble to confirm anomaly vs
 * noise. A clean keep escalates to a Tier-1 confirmer; a borderline keep goes straight to
 * Tier 2. Tier 1: one blind confirmer — both keep gives a keep; a lone dissent or an
 * unadjudicated confirmer escalates to Tier 2 (the C06 fix: one missing voter no longer
 * strands the candidate). Tier 2: finalised by `finaliseQuorum` only once the FULL ensemble
 * has reported; a partial feed dispatches the remaining voters rather than deciding early.
 */
