...
export function metaPrompt(clusters: readonly MetaCluster[]): string {
  return [
    ...
    '',
    'For EACH cluster below: use Glob/Grep/Read to byte-verify every member quote is still present, verbatim, at its stated file and line in the LIVE tree (grep -F the exact quote). If a quote has drifted or the line has moved, correct it where you can point at the live line; if you cannot, DROP that member by recording it in `droppedMembers` (its `id`, file, line, the quote as supplied, and the byte-verify reason) — never a metaNotes-only aside, never a silent omission. Emit `droppedMembers: []` when nothing was dropped. A row whose surviving instances fall below two — even to zero — is still emitted, with every drop named. Never trust the extracted quote unread.',
    "Every surviving instance AND every dropped member MUST carry its member's `id` copied VERBATIM from the cluster's instances — member conservation is recomputed in code as an exact id-set match (each cluster member id exactly once across survivors + drops); any missing, duplicated, or alien id fails the whole stage.",
    '',
