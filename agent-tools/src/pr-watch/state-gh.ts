...
import type { PrStateReading, ReviewRun, ReviewRunsLeg } from './states.js';

/**
 * The gh IO composition for `pr state`: one extended `pr view` call, the
 * review-threads GraphQL slurp (shared with `pr-watch`), and the
 * `gh agent-task` review-run legs, composed into one {@link PrStateReading}.
 *
 ...
 * in-flight run is mapped (liveness is the leg's whole point), completed runs
 * cap at {@link COMPLETED_RUN_MAP_LIMIT} most-recent. A missing or failing
 * `gh agent-task` command degrades the leg to a TYPED `unavailable` — the
 * verdict still computes and the gap is named in evidence, never silent.
 */
 ...
 *
 * @throws when the primary `pr view` or review-threads legs fail (a verdict
 *   without them would be a guess); only the agent-task leg degrades typed.
 */
export function readPrStateReading(options: ReadPrStateOptions): PrStateReading {
  ...
  return { ...view, reviewThreads, reviewRuns };
}
