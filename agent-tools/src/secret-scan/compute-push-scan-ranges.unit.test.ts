...
describe('computePushScanRanges', () => {
  it('scans local commits not on any remote when git supplies no ref lines (manual run)', () => {
    expect(computePushScanRanges({ refsText: '', remoteName: 'origin' })).toStrictEqual([
      'HEAD --not --remotes',
    ]);
  });
  ...
  it('falls back to all remotes for a new ref pushed to an unnamed URL', () => {
    const refsText = `refs/heads/x ${LOCAL} refs/heads/x ${ZERO}`;
    expect(computePushScanRanges({ refsText, remoteName: '' })).toStrictEqual([
      `${LOCAL} --not --remotes`,
    ]);
  });
